...
def commit_database_file(dataframe):
  try:
    ...
    st.toast("✅ Master System Updated!", icon="💾")
    return True
  except Exception as e:
...
st.markdown(
    ...
    <style>
        ...
        div[data-testid="stTextInput"] input, .stTextInput input, select {
...
# ==================== MODULE 5: REPAIR ====================
  ...
  with st.form("repair_form"):
    ...
    if st.form_submit_button("🛠️ UPDATE MAINTENANCE STATUS") and all_assets:
      df.loc[df["Asset Code"] == maint_asset, ["Status", "Remarks"]] = [
          maint_status,
          maint_remarks.strip(),
      ]
      if maint_status == "Available":
        df.loc[
            df["Asset Code"] == maint_asset, ["Assigned To", "Current Location"]
        ] = ["-", "MAIN STORE"]
      commit_database_file(df)
...
# ==================== MODULE 7: IMPORT & EXPORT DATA ====================
  ...
  st.markdown(
      "<div class='workspace-clean-card'><div class='card-heading'>📥 BULK"
      " IMPORT DATA</div>",
      unsafe_allow_html=True,
  )
  ...
  with col_imp1:
    ...
    if uploaded_file is not None:
      ...
      if st.button("🚀 UPDATE MASTER DATA NOW"):
        try:
          with st.spinner("Updating Master System..."):
            ...
            if "Merge" in import_mode:
              combined_df = pd.concat([df, imported_df], ignore_index=True)
            ...
            if success:
              st.success(
                  f"🎉 Successfully Saved {len(imported_df)} Records to"
                  " System!"
              )
              st.rerun()
        except Exception as e:
  ...
  st.markdown(
      "<div class='workspace-clean-card'><div class='card-heading'>📤 FULL"
      " BACKUP EXPORT (DATABASE DOWNLOAD)</div>",
      unsafe_allow_html=True,
  )
