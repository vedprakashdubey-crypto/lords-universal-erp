...
EXCEL_FILE = "college_assets.xlsx"
COLUMNS_LIST = [
    "Asset Code", "Asset Name", "Category", "Brand", "Model", "Serial Number",
    "Processor", "RAM", "Storage", "Operating System", "MAC Address", "IP Address",
    "Purchase Date", "Invoice Number", "Vendor", "Purchase Cost", "Warranty Start",
    "Warranty End", "Current Location", "Assigned To", "Department", "Status", "Remarks"
]
...
# FIXED CORE INITIALIZER: Automatically generates the database if it doesn't exist
def force_init_database():
    REAL_COLLEGE_DATA = [
        ["LUC-AIO-001", "Lenovo All-in-One", "Desktop", "Lenovo", "10113", "CSO1484482", "Intel Celeron CPU", "8 GB", "240 GB SSD", "Windows 10 64bit", "28:E3:47:00:A5:79", "192.168.6.201", "04-11-2014", "-", "-", "-", "-", "05-10-2015", "Deg Fee Counter", "Nasreen Sayed", "Degree admin", "Issued", "Good Condition"],
        ["LUC-AIO-002", "Lenovo All-in-One", "Desktop", "Lenovo", "10160", "CS02117081", "Intel Celeron J1800", "4 GB", "240 GB SSD", "Windows 10 64bit", "-", "-", "22-11-2014", "-", "-", "-", "-", "21-12-2015", "Jr Fee Counter", "Geeta Gupta", "Junior Admin", "Issued", "Good Condition"],
        ["LUC-PRN-001", "HP LaserJet Printer", "Printer", "HP", "Pro P1108 Plus", "CNCJB15890", "-", "-", "-", "-", "-", "-", "28-06-2024", "-", "-", "-", "-", "25-09-2025", "Admin Office", "Geeta Nandgaonkar", "Law admin", "Issued", "Good Condition"],
        ["LUC-AIO-003", "Lenovo All-in-One", "Desktop", "Lenovo", "10BBS03400", "PCO1DX0V", "Intel Core i5-4570S", "4 GB", "500 GB HDD", "Windows 10 64bit", "00:C2:C6:7C:BA:D7", "-", "31-10-2014", "-", "-", "-", "-", "30-10-2017", "Admin Office", "Shafali Chadha (CCTV)", "JR Principal", "Issued", "Good Condition"],
        ["LUC-AIO-004", "Dell Optiplex", "Desktop", "Dell", "Optiplex 9010", "325RVS1", "Intel Core i5-3450S", "8 GB", "500 GB HDD", "Windows 10 64bit", "-", "-", "2014-05-12", "-", "-", "-", "-", "2017-05-11", "Admin Office", "Principal Room", "Degree Principal", "Issued", "Working Fine"],
        ["LUC-LPT-001", "Lenovo Laptop", "Laptop", "Lenovo", "Lenovo 82TT", "PG04KFB6", "12th Gen Intel Core i5", "12 GB", "512 GB SSD", "Windows 10 64bit", "10-5F-AD-83-33-99", "192.168.11.1", "29-03-2025", "APT/24-25/1591", "APT INFOTECH", "65000", "29-03-2025", "09-12-2028", "IT LAB 101", "-", "IT", "Issued", "In Use"],
        ["LUC-LPT-002", "Dell Laptop", "Laptop", "Dell", "Latitude E5450", "1CN4Q32", "Intel Core i5-5300U", "8 GB", "256 GB SSD", "Windows 10 64bit", "34:02:86:46:51:4F", "192.168.6.211", "-", "-", "-", "-", "Horn", "-", "Calling Team-406", "Jeenal Gehlot", "IT", "Issued", "In Use"],
        ["LUC-LPT-003", "Dell Laptop", "Laptop", "Dell", "Latitude E5450", "629GH92", "Intel Core i5-5300U", "8 GB", "256 GB SSD", "Windows 10 64bit", "18:5E:0F:AB:8F:56", "192.168.6.212", "-", "-", "-", "Horn", "-", "-", "Calling Team-406", "Hadil Padiyar", "IT", "Issued", "In Use"]
    ]
...
def load_database_file():
    try:
        ...
        if data.empty:
            force_init_database()
            data = pd.read_excel(EXCEL_FILE)
        data = data.fillna("-").replace(["NA", "na", "NaN", "null", "NONE", "None"], "-")
...
def generate_product_prefix(category_str):
    clean = category_str.strip().upper()
...
with st.sidebar:
    st.markdown("<div style='padding: 10px 0; border-bottom: 1px solid #334155;'><h2 style='color:#FFFFFF; font-weight:800; font-size:18px; margin:0;'>LORDS UNIVERSAL</h2><p style='color:#38BDF8; font-size:11px; font-weight:600; margin:0;'>IT Asset Control ERP</p></div><br>", unsafe_allow_html=True)
    menu_selection = st.sidebar.radio(
        "NAVIGATION REGISTERS:",
        ["📊 Dashboard", "➕ Add New Asset", "✏️ Edit / Update Asset", "📑 Issue / Allocate Item", "🛠️ Repair & Maintenance", "⏱️ Warranty Records"],
        label_visibility="collapsed"
    )
    if menu_selection != "📊 Dashboard":
        st.session_state.current_dashboard_view = "Main_Grid"

st.markdown(f"<div class='workspace-clean-card' style='background: linear-gradient(90deg, #1E293B, #0F172A); border-color:#334155; padding: 16px 24px; margin-bottom:20px;'><span style='font-size:20px; font-weight:800; color:#FFFFFF;'>{menu_selection[2:].upper()} PANEL</span></div>", unsafe_allow_html=True)
...
def render_comprehensive_ledger(dataframe):
    ...
    for _, row in dataframe.iterrows():
        ...
        for col in COLUMNS_LIST:
            val = row[col]
            if col == "Status":
                if str(val).lower() in ["available", "working"]: html += f"<td><span class='status-pill pill-available'>{val}</span></td>"
                elif str(val).lower() in ["issued", "in use"]: html += f"<td><span class='status-pill pill-issued'>{val}</span></td>"
                elif "repair" in str(val).lower(): html += f"<td><span class='status-pill pill-repair'>{val}</span></td>"
                elif str(val).lower() in ["scrap", "damaged"]: html += f"<td><span class='status-pill pill-scrap'>{val}</span></td>"
...
if menu_selection == "📊 Dashboard":
    if st.session_state.current_dashboard_view == "Main_Grid":
        ...
        with m1:
            st.markdown(f"<div class='metric-card-wrapper'><span class='card-label'>Total Items</span><div class='card-val'>{t_count}</div></div>", unsafe_allow_html=True)
            if st.button("View Details ➡️", key="go_total"): st.session_state.current_dashboard_view = "Total_View"; st.rerun()
        with m2:
            st.markdown(f"<div class='metric-card-wrapper'><span class='card-label' style='color:#34D399;'>Available</span><div class='card-val' style='color:#34D399;'>{a_count}</div></div>", unsafe_allow_html=True)
            if st.button("View Details ➡️", key="go_avail"): st.session_state.current_dashboard_view = "Available_View"; st.rerun()
        with m3:
            st.markdown(f"<div class='metric-card-wrapper'><span class='card-label' style='color:#FBBF24;'>Issued</span><div class='card-val' style='color:#FBBF24;'>{i_count}</div></div>", unsafe_allow_html=True)
            if st.button("View Details ➡️", key="go_issued"): st.session_state.current_dashboard_view = "Issued_View"; st.rerun()
        with m4:
            st.markdown(f"<div class='metric-card-wrapper'><span class='card-label' style='color:#60A5FA;'>In Repair</span><div class='card-val' style='color:#60A5FA;'>{r_count}</div></div>", unsafe_allow_html=True)
            if st.button("View Details ➡️", key="go_repair"): st.session_state.current_dashboard_view = "Repair_View"; st.rerun()
        with m5:
            st.markdown(f"<div class='metric-card-wrapper'><span class='card-label' style='color:#F87171;'>Scrap Yard</span><div class='card-val' style='color:#F87171;'>{s_count}</div></div>", unsafe_allow_html=True)
            if st.button("View Details ➡️", key="go_scrap"): st.session_state.current_dashboard_view = "Scrap_View"; st.rerun()

        # 🔥 NEW FEATURE: CATEGORY WISE SUMMARY GRID PANEL 🔥
        st.markdown("<div class='workspace-clean-card'>", unsafe_allow_html=True)
        st.markdown("<div class='card-heading'>📂 Category-Wise Inventory Status</div>", unsafe_allow_html=True)
        
        if not df.empty:
            # Dynamic counting using pandas pivot logic
            cat_groups = df.groupby('Category').agg(
                Total=('Asset Code', 'count'),
                Available=('Status', lambda x: x.str.lower().isin(["available", "working"]).sum()),
                Issued=('Status', lambda x: x.str.lower().isin(["issued", "in use"]).sum()),
                In_Repair=('Status', lambda x: x.str.lower().str.contains("repair").sum()),
                Scrap=('Status', lambda x: x.str.lower().isin(["scrap", "damaged"]).sum())
            ).reset_index()
            
            # Custom styled category metrics grid table
        ...
        st.markdown("<div class='workspace-clean-card'>", unsafe_allow_html=True)
        st.markdown("<div class='card-heading'>📊 Quick Breakdown Filters</div>", unsafe_allow_html=True)
        b1, b2, b3 = st.columns(3)
        selected_cat = b1.selectbox("Filter by Category:", ["All Categories"] + sorted(df["Category"].unique().tolist()))
        selected_loc = b2.selectbox("Filter by Location:", ["All Locations"] + sorted(df["Current Location"].unique().tolist()))
        selected_assign = b3.selectbox("Filter by Assigned To:", ["All Personnel"] + sorted(df["Assigned To"].unique().tolist()))
        st.markdown("</div>", unsafe_allow_html=True)

        st.markdown("<div class='workspace-clean-card'>", unsafe_allow_html=True)
        fc1, fc2, fc3 = st.columns([1.5, 2, 1])
        grid_filter = fc1.selectbox("Select Status Filter:", ["All Items Pool", "Available Stock", "Issued / Deployed", "In Repair Pipeline", "Scrap Yard Stock"])
        ...
        if selected_cat != "All Categories": df_view = df_view[df_view["Category"] == selected_cat]
        if selected_loc != "All Locations": df_view = df_view[df_view["Current Location"] == selected_loc]
        if selected_assign != "All Personnel": df_view = df_view[df_view["Assigned To"] == selected_assign]
    ...
    with st.form("issue_form"):
        ...
        if st.form_submit_button("⚡ ASSIGN / ISSUE NOW") and available_assets:
            if assign_user.strip() and target_loc.strip():
                df.loc[df["Asset Code"] == target_asset, ["Assigned To", "Current Location", "Department", "Status", "Remarks"]] = [assign_user.strip(), target_loc.strip(), target_dept.strip(), "Issued", issue_remarks.strip()]
                commit_database_file(df)
